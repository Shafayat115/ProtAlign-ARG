...
# Device / runtime
GPU_INDEX: int = 0              # which GPU to use (0-based). Set to 1 for the 2nd GPU, etc.
USE_XLA: bool = False           # if True, keep XLA JIT; if False, disable to avoid device routing issues
SEED: int = 42                  # random seed for reproducibility

# Files & paths
EMBEDDING_TRAIN_FILENAME: str = "./Embeddings/ARG_Class_Beta_Train.h5"
SEQ_TRAIN_PATH: str            = "./Data/ARG_Class_Beta_Train.fasta"
MODEL_OUT_PATH: str            = "./Models/ARG_Class_Beta_Train.h5"
...
LOSS_FN: str = "binary_crossentropy"        # pairs with 'sigmoid' for multi-label/1-hot targets
...
from sklearn.metrics import f1_score, precision_score, recall_score, confusion_matrix  # (unused here; kept if you add eval)
...
# ----------------------------- Label maps -----------------------------
index_of_resistant_mechanisms = dict(LABEL_MAP)
...
# ----------------------------- Helpers -----------------------------
def create_lablels_of_protein_seq(args: str):
    """Build a multi-hot vector from the last |field in FASTA header."""
    label = [0] * NUM_CLASSES
        ...
        for k in keys:
            if k not in gene_id_label_dict:
                if RAISE_ON_ID_MISMATCH:
                    raise KeyError(f"ID '{k}' in embeddings has no label in FASTA")
                else:
                    continue
            train_x_embeddings_per_protein.append(f[k][:])
            train_y.append(gene_id_label_dict[k])

    train_x_embeddings_per_protein = np.array(train_x_embeddings_per_protein, dtype=np.float32)
    train_y = np.array(train_y, dtype=np.float32)

    if train_x_embeddings_per_protein.size == 0:
        raise RuntimeError("No (embedding, label) pairs found. Check that H5 keys match FASTA IDs.")

    # Shuffle
    X, Y = shuffle(train_x_embeddings_per_protein, train_y, random_state=SEED)
